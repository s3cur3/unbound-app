...
class SystemAppHelper {
  ...
  class func editorAppsForFileUrl(path: URL) -> [AppInfo] {
    let apps = LSCopyApplicationURLsForURL(path as CFURL, .editor)!.takeUnretainedValue() as? [URL] ?? []
    let defaultApp = LSCopyDefaultApplicationURLForURL(path as CFURL, .editor, nil)?.takeUnretainedValue() as URL?
