...
static NSString *kPhotoEntityName = @"Photo";
static NSString *kAlbumEntityName = @"Album";
...
-(IBAction)testFetchAllPhotosViaSpotlight:(id)sender
{
    [self.spotLightFetchController testFetchAllPhotos:sender];
}
...
-(IBAction)deleteAllRecords:(id)sender
{
    ...
    int counter = 0;
    NSError *error = nil;
    for (Photo *aPhoto in fetchedObjects)
    {
...
-(void)updateAlbumsPhotos
{
    ...
    for (Album *anAlbum in albumsToUpdate)
    {
